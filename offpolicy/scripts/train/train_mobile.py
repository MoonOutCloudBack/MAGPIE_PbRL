...
from offpolicy.envs.mobile.mobile_production_env import MobileProductionEnv
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            assert all_args.env_name == "mobile"
            env = MobileProductionEnv(all_args)
...
def parse_args(args, parser):
    parser.add_argument('--scenario_name', type=str, default='default', help="Which scenario to run on")
    parser.add_argument('--num_agents', type=int, default=3)
    parser.add_argument('--task_arrival_rate', type=float, default=3.0)
...
def main(args):
    ...
    if all_args.use_PbRL:
        all_args.experiment_name += f"_{all_args.size_segment}"
    run_dir = Path(os.path.split(os.path.dirname(os.path.abspath(__file__)))[0] + "/results"
        ) / all_args.env_name / all_args.scenario_name / all_args.algorithm_name / all_args.experiment_name / ("sd" + str(all_args.seed))
