...
class RecReplayBuffer(object):
    def __init__(self, policy_info, policy_agents, buffer_size, episode_length, use_same_share_obs, use_avail_acts,
        ...
        self.total_num_agents = sum([len(policy_agents[p_id]) for p_id in policy_info.keys()])
    ...
    # MARK: relabel
    def relabel_with_predictor(self, predict_func, agg_fn=np.sum):
        relabel_batch_size = 50
        total_iter = int(self.policy_buffers['policy_0'].filled_i / relabel_batch_size)

        if self.policy_buffers['policy_0'].filled_i > relabel_batch_size * total_iter:
            total_iter += 1

        all_share_obs = self.policy_buffers['policy_0'].share_obs  # [episode_length + 1, filled_i, share_obs_dim]
        all_acts = np.concatenate([self.policy_buffers[p_id].acts[:, :, 0, :] for p_id in self.policy_info.keys()], 
                axis=-1)  # [episode_length, filled_i, num_agents * act_dim]
        all_rewards = np.zeros((self.policy_buffers['policy_0'].episode_length, self.policy_buffers['policy_0'].filled_i,
                1, self.total_num_agents), dtype=np.float32)  # [episode_length, filled_i, 1, num_agents]

        for index in range(total_iter):
            last_index = (index + 1) * relabel_batch_size
            if (index + 1) * relabel_batch_size > self.policy_buffers['policy_0'].filled_i:
                last_index = self.policy_buffers['policy_0'].filled_i

            obses = all_share_obs[:-1,   index * relabel_batch_size: last_index,   :]  # [episode_length, batch_size, share_obs_dim]
            actions = all_acts[:,   index * relabel_batch_size: last_index,   :]  # [episode_length, batch_size, num_agents * act_dim]
            pred_reward = predict_func(obses, actions)  # [episode_length, batch_size, num_agents]
            all_rewards[:, index * relabel_batch_size: last_index, 0, :] = pred_reward

        # aggregate per-agent rewards: [episode_length, filled_i, 1, num_agents] -> [episode_length, filled_i, 1]
        all_rewards = agg_fn(all_rewards, axis=-1)  # [episode_length, filled_i, 1]

        for policy_idx, p_id in enumerate(self.policy_info.keys()):
            self.policy_buffers[p_id].rewards[:, :self.policy_buffers[p_id].filled_i, :, 0] = \
                    all_rewards[:, :, 0: 1].copy()
            # [episode_length, filled_i, num_agents, 1] <- [episode_length, filled_i, 1]
...
class RecPolicyBuffer(object):
    def __init__(self, buffer_size, episode_length, num_agents, obs_space, share_obs_space, act_space,
        ...
        if self.use_same_share_obs:  # MPE 的默认参数是 True
    ...
    def insert(self, num_insert_episodes, obs, share_obs, acts, rewards, dones, dones_env, avail_acts=None):
        ...
        self.share_obs[:, idx_range] = share_obs.copy()  # [episode_length + 1, buffer_size, share_obs_dim]
        self.acts[:, idx_range] = acts.copy()  # [episode_length, buffer_size, agent_num, act_dim] <- [episode_length, buffer_size, 1, act_dim]
        self.rewards[:, idx_range] = rewards.copy()  # [episode_length, buffer_size, agent_num, 1] <- [episode_length, buffer_size, 1, 1]
