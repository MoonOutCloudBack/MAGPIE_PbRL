...
class MPERunner(RecRunner):
    ...
    def __init__(self, config):
        ...
        num_warmup_episodes = max((self.batch_size, self.args.num_random_episodes, 
                (self.num_interact // self.episode_length) + 1))
    ...
    def eval(self):
        ...
        for _ in range(self.args.num_eval_episodes):
            env_info = self.collecter( explore=False, training_episode=False, warmup=False)[0]
    ...
    def shared_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        """
        Note tha we don't use this, as we don't use share_policy.
        ...
        episode_personalized_rewards = {p_id : np.zeros((self.episode_length, self.num_envs, self.num_agents, 1), dtype=np.float32) for p_id in self.policy_ids}
        ...
        if explore:
            ...
            rollout_info = {
                ...
                'episode_share_obs': episode_share_obs,
                'episode_acts': episode_acts,
                'episode_rewards': episode_rewards,
                'episode_dones': episode_dones,
                'episode_dones_env': episode_dones_env,
    ...
    def separated_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        episode_personalized_rewards = {p_id : np.zeros((self.episode_length, self.num_envs, len(self.policy_agents[p_id]), 1), dtype=np.float32) for p_id in self.policy_ids}
        ...
        while t < self.episode_length:
            for agent_id, p_id in zip(self.agent_ids, self.policy_ids):
                ...
                share_obs = np.concatenate([obs[0, i] for i in range(self.num_agents)\
                                            ]).reshape(self.num_envs, -1).astype(np.float32)
                # !!! 这里乱改的，不确定
                # share_obs = obs.reshape(self.num_envs, -1)
                # get actions for all agents to step the env
                if warmup:
            ...
            # env step and store the relevant episode information
            next_obs, rewards, dones, infos = env.step(env_acts)
            # reward shape: (num_envs, num_agents, 1)
            # info 格式：[[{'individual_reward': -2.132996254284449, 'personalized_reward': -5.278562512594337}
            #             {'individual_reward': -2.132996254284449, 'personalized_reward': -5.404080300163871}]]
            personalized_rewards = np.array([[[infos[env_idx][agent_idx]['personalized_reward']] \
                    for agent_idx in range(self.num_agents)] for env_idx in range(self.num_envs)])  # shape: (num_envs, num_agents, 1)
            ...
            for agent_id, p_id in zip(self.agent_ids, self.policy_ids):
                episode_rewards[p_id][t] = np.expand_dims(rewards[:, agent_id], axis=1)  # shape: (num_envs, 1, 1)
                episode_personalized_rewards[p_id][t] = np.expand_dims(personalized_rewards[:, agent_id], axis=1)
        ...
        if explore:
            ...
            rollout_info = {
                'episode_obs': episode_obs,
                'episode_share_obs': episode_share_obs,  # {pid: (episode_length + 1, num_envs, 1, share_obs_dim)}
                'episode_acts': episode_acts,  # {pid: (episode_length, num_envs, 1, act_dim)}
                'episode_rewards': episode_rewards,
                'episode_personalized_rewards': episode_personalized_rewards,
                'episode_dones': episode_dones,
                'episode_dones_env': episode_dones_env,
                'episode_avail_acts': episode_avail_acts
            }
    ...
    def log(self):
        ...
        self.env_infos['train_elapsed_time'] = [elapsed]
        print("\n Env {} Algo {} Exp {} runs total num timesteps {}/{}, FPS {}, elapsed {:.1f}s.\n"
              .format(self.args.scenario_name,
                      self.algorithm_name,
                      self.args.experiment_name,
                      self.total_env_steps,
                      self.num_env_steps,
                      int(self.total_env_steps / elapsed),
                      elapsed))
    ...
    def log_clear(self):
        """See parent class."""
        self.log_df.to_csv(self.log_df_dir + "/log.csv", index=False)
