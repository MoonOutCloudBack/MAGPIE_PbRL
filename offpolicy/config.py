...
def get_config():
    ...
    parser.add_argument('--num_env_steps', type=int,
                        default=3000000, help="Number of env steps to train for")
    ...
    parser.add_argument('--lr', type=float, default=7e-4,
                        help="Learning rate for Adam")  # original 5e-4
    ...
    parser.add_argument('--hard_update_interval_episode', type=int, default=100,  # original 200
    ...
    # PbRL parameters
    parser.add_argument('--use_PbRL', action='store_true', default=False, help="Whether to use PbRL")
    # max_feedback, num_interact, reward_update, reward_model_mb_size, size_segment
    parser.add_argument('--max_feedback', type=int, default=10000, help="Max feedback for PbRL")
    parser.add_argument('--num_interact', type=int, default=1000, help="Frequency of reward training")
    parser.add_argument('--reward_update', type=int, default=50, help="Reward update times")
    parser.add_argument('--reward_model_mb_size', type=int, default=100, help="Num of queries per collection")
    parser.add_argument('--reward_model_lr', type=float, default=3e-4, help="Learning rate for reward model")
    parser.add_argument('--size_segment', type=int, default=10, help="Segment length of PbRL")
    # L_reg_coef
    parser.add_argument('--L_reg_coef', type=float, default=2e-2, help="Reward regularization coefficient")
    # reward model type for PbRL baselines
    parser.add_argument('--reward_model_eps_mistake', type=float, default=0.0,
                        help="Label flip probability for noisy reward model variants "
                             "(global_noisy / mapt_noisy). 0.25 means 25%% of labels "
                             "are randomly flipped after unanimity-based noise. "
                             "0.0 (default) disables the flip, preserving original behavior.")
    parser.add_argument('--reward_model_type', type=str, default='magpie',
                        choices=['magpie'],
                        help="Reward model variant: magpie (default MAGPIE)")
